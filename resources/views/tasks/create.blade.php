<x-layout>
    <x-section>
        <x-form method="POST" action="{{ route('save_task') }}">
            <h3 class="mb-6 bg-gray-200 border-b border-gray-100 p-2">{{ __('Add New Task') }}</h3>
            <div class="mb-3">
                <label for="task_name" class="block mb-2 uppercase font-bold text-xs text-gray-700">{{ __('Task Name:') }}</label>
                <input type="text" id="task_name" name="task_name" class="border border-gray-200 p-2 w-full" value="{{ old('task_name') }}" placeholder="{{ __('Enter Task Name') }}">
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <textarea name="task_details" id="task_details" cols="30" rows="5" class="border border-gray-200 p-2 w-full" placeholder="{{ __('Enter Task Details') }}">{{ old('task_details') }}</textarea>
            </div>
            <div class="mb-3">
                <label for="task_deadline" class="block mb-2 uppercase font-bold text-xs text-gray-700">{{ __('Task Deadline:') }}</label>
                <input type="date" id="task_deadline" name="task_deadline" class="border border-gray-200 p-2 w-56" value="{{ old('task_deadline') }}">
            </div>
            <div class="mb-5">
